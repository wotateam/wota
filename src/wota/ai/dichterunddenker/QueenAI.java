...
public class QueenAI extends wota.gameobjects.HillAI {
	...
	public void tick() throws Exception {
		...
		talk(SeededRandomizer.getInt(360));
		/* 
		 ...
		 */
		if(endgame==false){
			if(SeededRandomizer.getInt(10)<9){
				createAnt(Caste.Gatherer, FamilieMann.class);
			}else{
				if(SeededRandomizer.getDouble()<0.6){
			...
			if(counter==55){
				counter=0;
				if(SeededRandomizer.getInt(10)<5){
					createAnt(Caste.Gatherer, FamilieMann.class);
				}else{
					if(SeededRandomizer.getDouble()<0.8){
						createAnt(Caste.Soldier, FranzKafka.class);
						createAnt(Caste.Soldier, FranzKafka.class);
						createAnt(Caste.Soldier, FranzKafka.class);
					}else{
